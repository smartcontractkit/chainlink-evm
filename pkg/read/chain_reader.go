...
func NewChainReaderService(_ context.Context, lggr logger.Logger, lp logpoller.LogPoller, ht logpoller.HeadTracker, client EVMClient, config config.ChainReaderConfig) (ChainReaderService, error) {
	...
	cr.bindings.SetBatchCaller(NewDynamicLimitedBatchCaller(
		cr.lggr,
		cr.codec,
		cr.client,
...
func (cr *chainReader) QueryKeys(ctx context.Context, filters []commontypes.ContractKeyFilter,
	...
	eventQueries := make([]EventQuery, 0, len(filters))
	for _, filter := range filters {
		...
		eventBinding, ok := binding.(*EventBinding)
		if !ok {
			return nil, fmt.Errorf("query key %s is not an event", filter.Key)
		}

		eventQueries = append(eventQueries, EventQuery{
	...
	return MultiEventTypeQuery(ctx, cr.lp, eventQueries, limitAndSort)
}
...
func (cr *chainReader) addMethod(
	...
	if err = cr.bindings.AddReader(contractName, methodName, NewMethodBinding(contractName, methodName, cr.client, cr.ht, confirmations, cr.lggr)); err != nil {
		return err
	}
...
func (cr *chainReader) addEvent(contractName, eventName string, a abi.ABI, chainReaderDefinition config.ChainReaderDefinition) error {
	...
	eb := NewEventBinding(contractName, eventName, cr.lp, event.ID, indexedTopicsCodecTypes, confirmations)
...
func (cr *chainReader) initTopicQuerying(contractName, eventName string, eventInputs abi.Arguments, genericTopicNames map[string]string, inputModifications commoncodec.ModifiersConfig) (map[string]TopicDetail, map[string]evmtypes.CodecEntry, map[string]commoncodec.Modifier, error) {
...
func (cr *chainReader) initDWQuerying(contractName, eventName string, abiDWsDetails map[string]DataWordDetail, cfgDWsDetails map[string]evm.DataWordDetail) (map[string]DataWordDetail, map[string]evmtypes.CodecEntry, error) {
...
func (cr *chainReader) constructDWDetails(cfgDWsDetails map[string]evm.DataWordDetail, abiDWsDetails map[string]DataWordDetail) (map[string]DataWordDetail, error) {
	...
	for genericName, cfgDWDetail := range cfgDWsDetails {
		...
		if cfgDWDetail.Index != nil {
			...
			dWsDetail[genericName] = DataWordDetail{Argument: abi.Argument{Type: abiTyp}, Index: *cfgDWDetail.Index}
		}
	}
...
func getEventTypes(event abi.Event) ([]abi.Argument, evmtypes.CodecEntry, map[string]DataWordDetail) {
...
func getDWIndexesWithTypes(eventName string, eventInputs abi.Arguments) map[string]DataWordDetail {
...
func processDWStaticField(fieldType abi.Type, parentFieldPath string, dataWords map[string]DataWordDetail, index int) int {
	switch fieldType.T {
	case abi.TupleTy:
	...
	default:
		dataWords[parentFieldPath] = DataWordDetail{
			Index:    index,
...
func processDWDynamicFields(eventName string, dataWords map[string]DataWordDetail, dynamicQueue []abi.Argument, dwIndex int) {
