...
func TestChainReader_Bind(t *testing.T) {
	...
	cr, err := read.NewChainReaderService(t.Context(), logger.Nop(), lp, ht, nil, config.ChainReaderConfig{Contracts: map[string]config.ChainContractReader{
		"test-contract": {
...
func (s *simpleTester) GetContractReader(t *testing.T) commontypes.ContractReader {
	...
	svc, err := read.NewChainReaderService(t.Context(), logger.Nop(), nil, new(simpleHeadTracker), client, config)
