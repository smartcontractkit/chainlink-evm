...
func newBroadcastLegacyEthTxAttempt(t testing.TB, etxID int64, gasPrice ...int64) txmgr.TxAttempt {
	attempt := testutils.NewLegacyEthTxAttempt(t, etxID)
...
func TestEthConfirmer_FindTxsRequiringRebroadcast(t *testing.T) {
	...
	testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, nonce, fromAddress)
	nonce++
	...
	etx1 := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, nonce, fromAddress)
	nonce++
	...
	etx2 := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, nonce, fromAddress)
	nonce++
	...
	etxWithoutAttempts := testutils.NewEthTx(fromAddress)
	{
		n := evmtypes.Nonce(nonce)
	...
	etx3 := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, nonce, fromAddress)
	nonce++
	...
	etxOther := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, 0, evmOtherAddress)
	...
	etx4 := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, nonce, fromAddress)
	nonce++
	...
	t.Run("ignores pending transactions for another key", func(t *testing.T) {
		...
		etxOther := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, (*etx3.Sequence).Int64(), evmOtherAddress)
	...
	// not be duplicated
	attempt4_2 := testutils.NewLegacyEthTxAttempt(t, etx4.ID)
...
func TestEthConfirmer_RebroadcastWhereNecessary_WithConnectivityCheck(t *testing.T) {
	...
	t.Run("should retry previous attempt if connectivity check failed for legacy transactions", func(t *testing.T) {
		...
		etx := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, nonce, fromAddress, originalBroadcastAt)
...
func TestEthConfirmer_RebroadcastWhereNecessary_TerminallyUnderpriced_ThenGoesThrough(t *testing.T) {
	...
	t.Run("multiple gas bumps with existing broadcast attempts are retried with more gas until success in legacy mode", func(t *testing.T) {
		...
		etx := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, nonce, fromAddress)
		nonce++
...
func TestEthConfirmer_ForceRebroadcast(t *testing.T) {
	...
	etx1 := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, 1, fromAddress)
	etx2 := testutils.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, 2, fromAddress)
