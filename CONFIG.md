...
SafeTagEnabled = true # Default
```
SafeTagEnabled means that the chain supports the safe block tag when querying for a block.
When FinalityTagEnabled is true and SafeTagEnabled is false, LatestSafeBlock will return the finalized block instead of safe block.
When SafeTagEnabled is true, LatestSafeBlock will return the safe block from the chain.
When FinalityTagEnabled is false, SafeTagEnabled is ignored and LatestSafeBlock uses SafeDepth.
