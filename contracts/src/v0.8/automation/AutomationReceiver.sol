...
contract AutomationReceiver is ReceiverTemplate, Pausable {
  ...
  event StaleReportSkipped(
    address indexed target, bytes4 indexed selector, uint256 reportBlockNumber, uint256 lastReportBlock
  );
  ...
  function getBlockNumberCheck(
    address target,
    bytes4 selector
  ) external view returns (bool enabled, uint256 lastReportBlock) {
  ...
  function _processReport(
    ...
    (address target, uint256 reportBlockNumber, bytes memory data) = abi.decode(report, (address, uint256, bytes));
