...
contract WorkflowRegistry_adminPauseAllByOwner is WorkflowRegistrySetup {
  function test_adminPauseAllByOwner_WhenCallerIsNOTTheContractOwner() external {
    ...
    s_registry.adminPauseAllByOwner(s_user, 0); // 0 means no limit
  }
  ...
  function test_adminPauseAllByDON_WhenThereAreActiveWorkflowsButLimitIsSet() external {
    ...
    vm.prank(s_owner);
    s_registry.adminPauseAllByOwner(s_user, 2); // pause first 2 workflows
    ...
    vm.prank(s_owner);
    s_registry.adminPauseAllByOwner(s_user, 2); // pause the remaining 2 workflows
