...
contract WorkflowRegistry_setUserDONOverride is WorkflowRegistrySetup {
  function test_setUserDONOverride_WhenTheCallerIsNOTTheContractOwner() external {
    ...
    s_registry.setUserDONOverride(s_stranger, s_donLabel, 100, true);
  }
  ...
  function test_setUserDONOverride_WhenNoPriorOverrideExistsForUserDonLabel() external {
    ...
    emit WorkflowRegistry.UserDONLimitSet(s_user, s_donLabel, 5);
  ...
  function test_setUserDONOverride_WhenNewLimitDoesNotEqualExistingOverrideValue() external {
    ...
    vm.expectEmit(true, true, true, false);
    emit WorkflowRegistry.UserDONLimitSet(s_user, s_donLabel, 20);
    s_registry.setUserDONOverride(s_user, s_donLabel, 20, true);
    assertEq(s_registry.getMaxWorkflowsPerUserDON(s_user, s_donLabel), 20);
  ...
  function test_setUserDONOverride_WhenLimitIsGreaterThanGlobalDONLimit() external {
    ...
    s_registry.setUserDONOverride(s_user, s_donLabel, 200, true);
  ...
  function test_setUserDONOverride_WhenAPriorOverrideExistsForUserDonLabel() external {
    ...
    // remove the limit
    vm.expectEmit(true, true, true, false);
    emit WorkflowRegistry.UserDONLimitUnset(s_user, s_donLabel);
    s_registry.setUserDONOverride(s_user, s_donLabel, 5, false);

    assertEq(s_registry.getMaxWorkflowsPerUserDON(s_user, s_donLabel), 100); // global don value
    vm.stopPrank();
  }
