...
contract WorkflowRegistry_setDONLimit is WorkflowRegistrySetup {
  ...
  function test_setDONLimit_WhenNoPreviousLimitExistsForDonLabel() external {
    ...
    emit WorkflowRegistry.DONLimitSet(s_donLabel, newLimit);

    s_registry.setDONLimit(s_donLabel, newLimit, true);
    uint32 donLimit = s_registry.getMaxWorkflowsPerDON(s_donLabel);
  ...
  function test_setDONLimit_WhenNewLimitDoesNotEqualExistingLimit() external {
    ...
    emit WorkflowRegistry.DONLimitSet(s_donLabel, newLimit);
    s_registry.setDONLimit(s_donLabel, newLimit, true);
    assertEq(s_registry.getMaxWorkflowsPerDON(s_donLabel), newLimit);
  ...
  function test_setDONLimit_WhenPreviousLimitExistsForDonLabel() external {
    ...
    vm.expectEmit(true, true, true, false);
    emit WorkflowRegistry.DONLimitSet(s_donLabel, 100);
    s_registry.setDONLimit(s_donLabel, 100, false);
