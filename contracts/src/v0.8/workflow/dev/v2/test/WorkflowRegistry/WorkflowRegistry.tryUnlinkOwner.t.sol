...
contract WorkflowRegistrytryUnlinkOwner is Test {
    ...
    givenThatTheOwnerSelectsNonePreAction
  {
    ...
    vm.prank(owner);
    // expect no revert
    wr.tryUnlinkOwner(owner, validityTimestamp, proof, sig, WorkflowRegistry.PreUnlinkAction.NONE);
    assertTrue(wr.isOwnerLinked(owner), "Owner should be still linked");
  }
