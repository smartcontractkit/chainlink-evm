...
interface ICapabilityConfiguration {
  ...
  /// @param mutableDONConfig The configuration being set on the capability registry.
  /// @param mutableDONConfigCount The number of times the DON has been configured, tracked on the capability registry.
  /// @param donId The DON ID on the capability registry.
  function beforeCapabilityConfigSet(
    bytes32[] calldata nodes,
    bytes calldata mutableDONConfig,
    uint64 mutableDONConfigCount,
    uint32 donId
  ) external;
}
